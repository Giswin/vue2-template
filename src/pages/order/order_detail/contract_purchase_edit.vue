...
<template>
    <vxe-modal ref="modal1" :value="show" :showFooter="true" width="1040" height="600" resize remember :mask-closable="false">
        <template v-slot:header>
            <span class="vxe-modal--title">{{contract_id==null?'采购合同-新建':'采购合同-编辑'}}</span>
                        ...
                        <table width="100%" border="0" cellspacing="0" cellpadding="0" class="definition">
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <el-select v-model="base_info.contract_type" style="width: 100%;" clearable @change="changeContractType">
                                            <el-option v-for="item in selectOptions.productType" :key="item.lookup_code" :value="item.lookup_code" :label="item.meaning">
                                            </el-option>
                                        </el-select>
                                    </td>
                                </tr>
                                <tr>
                                    ...
                                    <td>
                                        <el-date-picker value-format="yyyy-MM-dd" type="date" v-model="base_info.contract_date" placeholder="选择日期" size="small" clearable>
                                        </el-date-picker>
                                    </td>
                                </tr>
                                <tr>
                                    <th align="left"><span style="color: red;">*</span>接单抬头</th>
                                    <td>
                                ...
                                <tr>
                                    <th align="left"><span style="color: red;">*</span>币种</th>
                                    <td>
                        ...
                        <table width="100%" border="0" cellspacing="0" cellpadding="0" class="definition">
                            <tbody>
                                <tr>
                                    ...
                                    <td>
                                        <el-select v-model="base_info.contract_actual_supply_title_name" placeholder="请输入关键词选择" size="small" clearable filterable remote :remote-method="remoteMethod2" @change="changeVender" @clear="clearVender">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <el-input v-model="base_info.contract_actual_supply_address" clearable placeholder="请输入"></el-input>
                                    </td>
                                </tr>
                                <tr>
        ...
        <div v-show="selectTab == 1">
            ...
            <div style="margin: 0 10px;">
                <vxe-table ref="goodsTable" border resizable :data="detailInfo" size="small" auto-resize :column-config="{minWidth:130}" class="mytable-scrollbar" highlight-current-row highlight-hover-row show-footer :footer-method="footerMethod" :edit-config="{trigger: 'click', mode: 'cell'}" show-overflow :keyboard-config="{isArrow: true, isDel: true, isEnter: true, isTab: true, isEdit: true,editMethod1}" :mouse-config="{selected: true}">
                    ...
                    <vxe-table-column field="contract_detail_article_number" title="采购货号" width="100" align="center" :edit-render="{autofocus:'.el-input__inner',autoselect:true}">
                        <template #edit="{ row }">
                            <el-input v-model="row.contract_detail_article_number" size="small" clearable></el-input>
                            <!-- <el-select v-model="row.contract_detail_article_number" size="small" filterable clearable>
								<el-option v-for="item in articleNumOptions" :key="item.value" :value="item.value" :label="item.label"></el-option>
							</el-select> -->
                        </template>
                    </vxe-table-column>
                    <vxe-table-column field="contract_detail_name" title="采购品名" width="100" header-align="center" align="left" :edit-render="{autofocus:'.el-input__inner',autoselect:true}">
                        <template #edit="{ row }">
                    ...
                    <vxe-table-column field="contract_detail_unit" title="单位" width="70" header-align="center" align="right" :edit-render="{autofocus:'.el-input__inner',autoselect:true}">
                        <template #edit="{ row }">
                    ...
                    <vxe-table-column field="contract_detail_price" title="单价" width="100" header-align="center" align="right" :edit-render="{autofocus:'.el-input__inner',autoselect:true}">
                        <template #edit="{ row }">
        ...
        <div v-show="selectTab == 2">
            <div class="class-title" style="margin-top:10px">
                ...
                <div class="m-r" style="margin-top: -5px;padding-right:10px">
                    ...
                    <el-select size="mini" v-model="TK_order_memo" filterable remote clearable placeholder="复制其他合同条款" @clear="TK_order_memo=null;" :remote-method="remoteTkMethod" :loading="loading" @change="changeTKOrder">
                        <el-option v-for="item in contractCode" :key="item.odpc_id" :value="item.odpc_id" :label="item.odpc_code_ebs">
                        </el-option>
                    </el-select>
                    <el-select size="mini" placeholder="合同条款选择" v-model="TK_memo" @change="showTK">
                        <el-option v-for="item in TK" :value="item.dictionary_id" :label="item.dic_name"></el-option>
                    </el-select>
                </div>
            </div>
        ...
        <div v-show="selectTab == 3">
            ...
            <div style="margin: 0 10px;">
                <vxe-table ref="payTable" border resizable :data="base_info.contract_collect_clause" show-overflow size="small" auto-resize :column-config="{minWidth:120}" class="mytable-scrollbar" highlight-current-row highlight-hover-row :edit-config="{trigger: 'click', mode: 'cell',}" :keyboard-config="{isArrow: true, isDel: true, isEnter: true, isTab: true, isEdit: true,editMethod2}" :mouse-config="{selected: true}">
                    <vxe-table-column type="checkbox" align="center" width="40" fixed="left"></vxe-table-column>
            ...
            <el-row>
                <vxe-table ref="tkListData" :checkbox-config="{'checkAll':true}" border resizable highlight-cell :data="tkList" class="vxe-table-scrollbar" auto-resize :edit-config="{trigger: 'click', mode: 'cell'}">
                    ...
                    <vxe-table-column field="memo" title="条款" header-align="center" align="left">
                        <template v-slot="{row}">
                            <!-- <el-input type="textarea" v-model="row.memo" :autosize="{minRows:2,maxRows:3}" :disabled="row.description == '不可编辑'"></el-input> -->
                            <el-input type="textarea" v-model="row.memo" :autosize="{minRows:2,maxRows:3}"></el-input>
                        </template>
                    </vxe-table-column>
                </vxe-table>
            </el-row>
        ...
        <div slot="footer">
            ...
            <el-button size="small" v-if="contract_id != null" @click="purchasePublish">发布</el-button>
            <el-button size="small" v-if="contract_id != null" @click="purchasePrint">预览</el-button>
        </div>
    </vxe-modal>
</template>
...
export default {
    ...
    methods: {
        ...
        getContractCodeList() {
            projectApi.getContractCodeList("purchase").then((res) => {
                this.contractCodeList = res.data;
            });
        },
        getEBS() {
            ebsApi
                .getEbsGoods(this.goods_stock_org_id_ebs, this.ebs_path)
                .then((res) => {
        ...
        getStockId(depId) {
            ...
            if (this.ebs_path == "HS") {
                this.base_info.contract_stock_org_id_ebs =
                    this.goods_stock_org_id_ebs;
            }
            if (this.ebs_path == "HK") {
                this.base_info.contract_stock_org_id_ebs =
        ...
        getContractData() {
            purchaseApi.getContractInfo(this.contract_id).then((res) => {
                ...
                console.log(
                    "🚀 ~ file: contract_purchase_edit.vue:652 ~ purchaseApi.getContractInfo ~ this.base_info",
                    this.base_info
                );
        ...
        getExchgeList() {
            ...
            omApi.getExchangeRate(month, year).then((res) => {
                ...
                if (this.goodsInfo.length != 0) {
                    this.base_info.contract_currency =
                        this.goodsInfo[0].order_goods_bom_currency;
        ...
        getBaseInfo() {
            ...
            this.base_info.contract_category = "purchase";
            ...
            this.base_info.contract_salesman_name =
                this.orderData.order_salesman_name;
            this.base_info.contract_salesman_id =
                this.orderData.order_salesman_id;
            this.base_info.contract_title_name =
                this.orderData.order_title_name;
            this.base_info.contract_title_id = this.orderData.order_title_id;
            if (this.goodsInfo.length != 0) {
                this.base_info.contract_supply_title_name =
                    this.goodsInfo[0].order_goods_bom_ven_name;
                this.base_info.contract_supply_title_id =
                    this.goodsInfo[0].order_goods_bom_ven_id;
                this.base_info.contract_actual_supply_title_name =
                    this.goodsInfo[0].order_goods_bom_ven_name;
                this.base_info.contract_actual_supply_title_id =
                    this.goodsInfo[0].order_goods_bom_ven_id;
                this.base_info.contract_is_stock = 2;
                let goodsInfo = this.$general.deepClone(this.goodsInfo);
                goodsInfo.forEach((item) => {
                    this.detailInfo.push({
                        contract_detail_article_number:
                            item.order_goods_bom_article_number, //采购货号
                        ...
                        contract_detail_tax_rate:
                            this.$dmBaseSet.datalist.optionsData.VAT_RATE
                                .option_value, //税率
                        contract_detail_amount:
                            item.order_goods_bom_qty *
                            item.order_goods_bom_price, //金额小计
                        contract_detail_etd: null, //交货日期
                        contract_detail_goods_id:
                            item.order_goods_bom_archives_id, //商品id
                        contract_detail_goods_id_ebs:
                            item.order_goods_bom_ebs_id, //ebs商品id
                        contract_detail_goods_name_ebs:
                            item.order_goods_bom_ebs_name, //ebs商品名称
                        contract_detail_order_goods_bom_id:
                            item.order_goods_bom_id, //订单成本信息ID
                    });
                });
            } else {
            ...
            this.base_info.contract_project_id =
                this.orderData.order_project_id;
            this.base_info.contract_project_code =
        ...
        async getDic() {
            this.$dmBaseSet
                .dicData("TY_BZ,TY_JLDW,TY_HTTK-PU_I,TY_HTTK-PU_E")
                .then((data) => {
                    let pui = data["TY_HTTK-PU_I"].children || [];
                    let pue = data["TY_HTTK-PU_E"].children || [];

                    this.TY_JLDW =
                        this.$general.objectKeyBy(
                            data["TY_JLDW"].children,
                            "dic_name"
                        ) || [];
        ...
        //获取采购类型
        getProductType() {
            ebsApi.getEbsData("get_order_type", "", "HS").then((result) => {
                if (result.code == 0) {
        ...
        changeContractType(value) {
            if (value == "05") {
                this.base_info.contract_currency = "CNY";
                this.base_info.contract_ex_rate = this.exchangeList["CNY"];
                this.base_info.contract_sign_place =
                    this.$dmBaseSet.datalist.optionsData.SIGN_PLACE.option_value[1].value;
            } else if (value == "01") {
                this.base_info.contract_currency = "USD";
                this.base_info.contract_ex_rate = this.exchangeList["USD"];
                this.base_info.contract_sign_place =
                    this.$dmBaseSet.datalist.optionsData.SIGN_PLACE.option_value[0].value;
            }
        },
            ...
            if (value) {
                this.selectOptions.vender.forEach((item) => {
                    if (item.trader_name == value) {
                        ...
                        this.trader_id_ebs =
                            this.ebs_path == "HS"
                                ? Number(item.trader_id_ebs)
                                : Number(item.trader_id_ebs_hk);
                        this.base_info.trader_id_ebs =
                            this.ebs_path == "HS"
                                ? Number(item.trader_id_ebs)
                                : Number(item.trader_id_ebs_hk);
                    }
        ...
        clearVender() {
            ...
            this.base_info.contract_actual_supply_contract_info.b_legal_person_b =
                null;
            this.base_info.contract_actual_supply_contract_info.b_legal_person_e =
                null;
            this.base_info.contract_actual_supply_address = null;
            this.base_info.contract_actual_supply_contract_info.b_bank_name =
                null;
            this.base_info.contract_actual_supply_contract_info.b_bank_account_num =
                null;
            this.base_info.contract_actual_supply_tel = null;

            this.base_info.contract_supply_title_id = null;
            this.base_info.contract_supply_title_name = null;
            this.base_info.contract_supply_contract_info.b_legal_person_b =
                null;
            this.base_info.contract_supply_contract_info.b_legal_person_e =
                null;
            this.base_info.contract_supply_address = null;
            this.base_info.contract_supply_contract_info.b_bank_name = null;
            this.base_info.contract_supply_contract_info.b_bank_account_num =
                null;
            this.base_info.contract_supply_tel = null;
        },
                ...
                .then((result) => {
                    if (result.code == 0 && result.data != null) {
                        if (result.data.length != 0) {
                            ...
                            this.base_info.contract_supply_contract_info.b_bank_name =
                                venderBank.bank_branch_name;
                            this.base_info.contract_supply_contract_info.b_bank_account_num =
                                venderBank.bank_account_num;
                ...
                .then((result) => {
                    if (result.code == 0 && result.data != null) {
                        if (result.data.length != 0) {
                            ...
                            this.base_info.contract_supply_address =
                                venderBankAddress.address1;
                            this.base_info.contract_supply_tel =
                                venderBankAddress.contact_tel;

                            this.base_info.contract_actual_supply_address =
                                venderBankAddress.address1;
                            this.base_info.contract_actual_supply_tel =
                                venderBankAddress.contact_tel;
                        }
                    }
                });
        ...
        async getCustomerT() {
            await ebsApi
                .getEbsData("get_venlist_id", this.trader_id_ebs, this.ebs_path)
                .then((result) => {
                    if (result.code == 0 && result.data != null) {
                        if (result.data.length != 0) {
                            let customerT = result.data || [];
                            this.base_info.contract_supply_contract_info.b_legal_person_b =
                                customerT[0].legal_person;
                            this.base_info.contract_supply_contract_info.b_legal_person_e =
                                customerT[0].legal_person;
        ...
        changeEBSGoods(row) {
            this.ebsGoods.forEach((item) => {
                if (
                    item.inventory_item_id == row.contract_detail_goods_id_ebs
                ) {
        ...
        changeAmount(row) {
            row.contract_detail_amount =
                row.contract_detail_qty * row.contract_detail_price;
        },
                ...
                this.$confirm("确定要删除付款方式?", "提示", {
                    ...
                    .then(() => {
                        for (let m = selectRecords.length - 1; m >= 0; m--) {
                            let row = this.$refs.payTable.getRowIndex(
                                selectRecords[m]
                            );
                            this.base_info.contract_collect_clause.splice(
                                row,
                                1
                            );
                        }
        ...
        delGoodsInfo() {
            ...
            if (selectRecords.length > 0) {
                this.$confirm("确定要删除商品信息?", "提示", {
                    ...
                    .then(() => {
                        for (let m = selectRecords.length - 1; m >= 0; m--) {
                            let row = this.$refs.goodsTable.getRowIndex(
                                selectRecords[m]
                            );
        ...
        changeTK(memo) {
            let TK = memo;
            this.base_info.contract_clause = TK;
            this.tkModal = false;
            this.editBasicInfo = false;
        },

        //条款内容变更
        onContentChange(val, val2) {
            if (val2.children[0].children[0].text != "") {
                this.base_info.contract_clause = val;
            } else {
                this.base_info.contract_clause = "";
            }
        },

        //获取采购合同付款类型
        getPaymentType() {
            ebsApi.getEbsData("get_payment_type", "", "HS").then((result) => {
                //this.ebs_path 'HK''HS'
                if (result.code == 0) {
        ...
        saveContract() {
            ...
            if (this.detailInfo.length > 0) {
                ...
                if (this.base_info.contract_collect_clause.length > 0) {
                    ...
                    for (
                        let i = 0;
                        i < this.base_info.contract_collect_clause.length;
                        i++
                    ) {
                        if (
                            this.base_info.contract_collect_clause[i]
                    ...
                    purchaseApi
                        .updateContractInfo(this.base_info, this.detailInfo)
                        .then((res) => {
                            if (res.code == 0) {
                                this.$message({
                                    message: "保存采购合同成功！",
                                ...
                                this.$emit("afterSavePurContract");
        ...
        getSelectBom(bomData) {
            this.base_info.contract_actual_supply_title_name =
                bomData[0].order_goods_bom_ven_name;
            if (this.base_info.contract_actual_supply_title_name != null) {
                this.changeVender(
                    this.base_info.contract_actual_supply_title_name
                );
                ...
                this.detailInfo.push({
                    ...
                    contract_detail_tax_rate:
                        this.$dmBaseSet.datalist.optionsData.VAT_RATE
                            .option_value, //税率
                    contract_detail_amount:
                        item.order_goods_bom_qty * item.order_goods_bom_price, //金额小计
                    contract_detail_etd: null, //交货日期
                    contract_detail_goods_id: item.order_goods_bom_archives_id, //商品id
        ...
        purchasePrint() {
            window.open(
                "/#/order_purchase_contract_print?contract_id=" +
                    this.contract_id +
                    "&flag=preview"
            );
        },
            ...
            this.$confirm("是否要替换该合同条款？", "提示", {
                ...
                .then(() => {
                    if (Array.isArray(this.tkList)) {
                        if (this.tkList.length > 0) {
                            this.clauseList = [];
                            this.clauseList = this.$general.deepClone(
                                this.tkList
                            );
                            this.clauseList.forEach((item) => {
                                item.memo =
