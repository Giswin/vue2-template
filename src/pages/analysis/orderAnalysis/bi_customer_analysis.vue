            ...
            <div class="bi-body of">
                ...
                <div class="filter-analysis">
                    <div style="margin-bottom: 8px">
                        ...
                        <div class="m-l" style="margin-left: 15px" v-show="selectOption.order_dep_name">
                            部门:<el-tag size="mini" v-for="(item,index) in selectOption.order_dep_name.value " :key="index" style="margin-left: 15px">{{ item }}</el-tag>
                        </div>
                        <div class="m-l" style="margin-left: 15px">
                            接单日期：<el-tag size="mini" v-show="date.order_date_start">{{ date.order_date_start }}</el-tag>
                            <el-tag style="margin-left: 15px" size="mini" v-show="date.order_date_end">{{ date.order_date_end }}</el-tag>
                        </div>
                        ...
                        <tbody>
                            <tr>
                                <td>
                                    ...
                                    <el-tooltip content="订单数" placement="bottom" effect="light">
                                        <i class="el-icon-question"></i>
                                    </el-tooltip>
                                    <div class="value" v-if="curr_customer.order_num">
                                        {{trans_orderNum}}
                                    </div>
                                </td>
                                <td>
                                    <span class="text">订单件数</span>
                                    <el-tooltip content="订单件数" placement="bottom" effect="light">
                                        <i class="el-icon-question"></i>
                                    </el-tooltip>
                                    <div class="value" v-if="curr_customer.order_qty">
                                        {{ trans_orderQty }}
                                    </div>
                                </td>
                                <td>
                                    <span class="text">订单金额(CNY)(万)</span>
                                    <el-tooltip content="订单金额(CNY)(人民币,单位:万)" placement="bottom" effect="light">
                                        <i class="el-icon-question"></i>
                                    </el-tooltip>
                                    <div class="value" v-if="curr_customer.order_amount_cny">
                                        {{ trans_cny}}
                                    </div>
                                </td>
                                <td>
                                    <span class="text">订单金额(USD)(万)</span>
                                    <el-tooltip content="订单金额(USD)(美元,单位:万)" placement="bottom" effect="light">
                                        <i class="el-icon-question"></i>
                                    </el-tooltip>
                                    <div class="value" v-if="curr_customer.order_amount_usd">
                                        {{ trans_usd}}
                                    </div>
                                </td>
                                <td>
                                    <span class="text">金额占比</span>
                                    <el-tooltip content="占比=折USD金额 /总折USD金额" placement="bottom" effect="light">
                                        <i class="el-icon-question"></i>
                                    </el-tooltip>
...
export default {
    ...
    created() {
        ...
        console.log("curr_customer", this.curr_customer);
    ...
    computed: {
        trans_orderNum() {
            return this.$general.transform(this.curr_customer.order_num, 0);
        },
        trans_orderQty() {
            return this.$general.transform(this.curr_customer.order_qty, 0);
        },
        trans_cny() {
            return this.$general.transform(
                this.curr_customer.order_amount_cny,
                2
            );
        },
        trans_usd() {
            return this.$general.transform(
                this.curr_customer.order_amount_usd,
                2
            );
        },
        // percent() {
        //     return (this.curr_customer.proportion * 100).toFixed(2) + "%";
        // },
    },
    methods: {
        closeModel() {
        ...
        search() {
            this.pieList.forEach((item) => {
                if (
                    item.order_cus_name.toLowerCase() ==
            ...
            let condition = {
                order_cus_id: this.curr_customer.order_cus_id,
        ...
        getPieData() {
            ...
            console.log("miniPie", this.miniPie);
        },
        querySearchAsync(queryString, cb) {
            var array = [];
            this.pieList.forEach((item) => {
                array.push({ value: item.order_cus_name });
            });
            if (array.length > 50) {
                array.pop();
            }
