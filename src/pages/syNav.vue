...
export default {
    ...
    methods: {
        ...
        //根据store里language的值显示菜单
        changeNav(){
            let menuTitle=this.$t('menu')
            for(let i in this.menu_list){
                this.menu_list[i].title=menuTitle[i].title
                if(this.menu_list[i].children!=undefined){
                    for(let j in this.menu_list[i].children){
                        this.menu_list[i].children[j].title=menuTitle[i].childrenTitle[j]
                    }
                }
            }
