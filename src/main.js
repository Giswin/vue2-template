...
//在页面刷新时读取sessionStorage里的语言信息
if(JSON.parse(sessionStorage.getItem("language"))!=null){
  store.state.language=JSON.parse(sessionStorage.getItem("language"))
}
...
router.beforeEach((to, from, next) => {
  if(store.state.language=='cn'){
    ...
    if (to.meta.englishTitle) {
      document.title = "ShangYouSoft - " + to.meta.englishTitle;
    } else {
      document.title = "ShangYouSoft";
    }
  }
